...
object* make_final() {
    ...
    list[l++] = boundary;
    list[l++] = new constant_medium(boundary, 0.2f, new constant_texture(glm::vec3(0.2, 0.4, 0.9)));
    boundary = new sphere(glm::vec3(0, 0, 0), 5000, new dielectric(1.5));
    list[l++] = new constant_medium(boundary, 0.0001f, new constant_texture(glm::vec3(1.0, 1.0, 1.0)));
    int nx, ny, nn;
    unsigned char* tex_data =
        stbi_load("E:/Projects/ray_tracing_in_a_weekend/physical_earth_satellite_image_mural_lg.jpg", &nx, &ny, &nn, 0);
